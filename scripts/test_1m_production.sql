-- ============================================================================
-- 1 MILLION EVENT PRODUCTION TEST WITH 3-BUFFER CONFIGURATION
-- Baseline performance validation before load balancing
-- ============================================================================
...
SELECT '============================================' as separator;
SELECT '1 MILLION EVENT PRODUCTION TEST' as test_name;
...
-- ============================================================================
-- DISTRIBUTION PLAN:
-- - 85% Real Users (850K) -> Buffers 1 & 3
-- - 15% Bots (150K) -> Buffer 2
-- ============================================================================

SELECT 'Starting data insertion...' as status;

-- ============================================================================
-- BUFFER 1: 425K Real User Events (42.5% of total)
...
SELECT
    ...
    -- Simulate 1000 different links (some viral, some normal)
    toUUID(concat('00000000-0000-0000-0000-', lpad(toString(
        CASE 
            ...
            ELSE toUInt32(rand() % 1000)  -- 70% spread across 1000 links
        END
    ), 12, '0'))) as link_id,
...
SELECT now() as buffer1_complete, '425K events inserted to Buffer 1' as status;

-- ============================================================================
-- BUFFER 2: 150K Bot Events (15% of total)
...
SELECT
    ...
    toUUID(concat('00000000-0000-0000-0000-', lpad(toString(toUInt32(rand() % 50)), 12, '0'))) as link_id,
...
SELECT now() as buffer2_complete, '150K bot events inserted to Buffer 2' as status;

-- ============================================================================
-- BUFFER 3: 425K Real User Events (42.5% of total)
...
SELECT
    ...
    toUUID(concat('00000000-0000-0000-0000-', lpad(toString(425000 + number), 12, '0'))) as user_id,
    now() - toIntervalSecond(rand() % 300) as timestamp,  -- Last 5 minutes
    today() as date,
    toIPv6(IPv4NumToString(toUInt32(rand()))) as ip_address,
...
SELECT now() as buffer3_complete, '425K events inserted to Buffer 3' as status;
SELECT now() as insertion_complete, '1M total events inserted' as status;
...
SELECT '============================================' as separator;
SELECT '1M EVENT TEST - FINAL RESULTS' as report_title;
SELECT '============================================' as separator;
...
SELECT '============================================' as separator;
SELECT '✅ 1M EVENT TEST COMPLETED SUCCESSFULLY' as status;
SELECT '✅ BOT TRAFFIC PROPERLY ISOLATED' as isolation;
SELECT '✅ BASELINE PERFORMANCE ESTABLISHED' as validation;
SELECT '✅ READY FOR LOAD BALANCER TESTING' as conclusion;
SELECT '============================================' as separator;