...
pub async fn register(
        ...
        .require_email_verification
    {
        ...
        if let Err(e) = verification_service
            .store_verification_code(&created_user.email, &created_user.id.to_string(), &code)
            .await
        {
            ...
            match EmailService::new(config.email.clone()) {
                Ok(email_service) => {
                    match email_service
                    ...
                    {
                        Ok(_) => {
                            tracing::info!("Verification email sent to: {}", created_user.email);
                            true
                        },
                        Err(e) => {
        ...
        false // Email verification not required
    };
...
pub async fn verify_email(
    ...
    if request.code.len() != 6 || !request.code.chars().all(|c| c.is_ascii_digit()) {
        return AuthError::ValidationError("Invalid verification code format".to_string())
            .into_response();
    }
    ...
    // Verify the code
    let user_id = match verification_service
        .verify_code(&email, &request.code)
        .await
    {
        Ok(id) => id,
        Err(e) => {
            tracing::warn!("Email verification failed for {}: {:?}", email, e);
            let error_msg = match e {
                crate::services::verification::VerificationError::InvalidCode => {
                    "Invalid verification code"
                },
                crate::services::verification::VerificationError::CodeExpired => {
            ...
            return AuthError::ValidationError(error_msg.to_string()).into_response();
        },
    };
    ...
    let mut conn = match state.diesel_pool.get().await {
        ...
        Err(e) => {
            tracing::error!("Failed to get database connection: {}", e);
            return AuthError::DatabaseError("Failed to get database connection".to_string())
                .into_response();
        },
    };
    ...
    let user_uuid = match uuid::Uuid::parse_str(&user_id) {
        Ok(uuid) => uuid,
        Err(_) => {
            return AuthError::ValidationError("Invalid user ID".to_string()).into_response();
        },
    };

    let result = diesel::update(dsl::users.filter(dsl::id.eq(user_uuid)))
        .set((
            dsl::email_verified.eq(true),
            dsl::email_verified_at.eq(Some(chrono::Utc::now())),
            dsl::onboarding_status.eq(OnboardingStatus::Verified.as_str()),
        ))
        .execute(&mut conn)
        .await;
    ...
    match result {
        Ok(0) => {
            tracing::error!("User not found for email verification: {}", user_id);
            AuthError::UserNotFound.into_response()
        },
        Ok(_) => {
            // Send welcome email
            let email_service = EmailService::new(config.email.clone()).unwrap();

            // Get user details for welcome email
            let user: User = dsl::users
                .filter(dsl::id.eq(user_uuid))
                .first(&mut conn)
                .await
                .unwrap();
            ...
            // Generate refresh token (no device info from email verification)
            let refresh_token = match state
                .jwt_service
                .generate_refresh_token_with_device(
                    &user.id.to_string(),
                    None, // No device fingerprint from email click
                    None, // No IP address tracking for email verification
                    None, // No user agent from email client
                )
                .await
            {
                Ok(token) => token,
            ...
            tracing::info!(
                "Email verified successfully for user: {}, auto-login tokens generated",
                user_id
            );
            ...
            let response = AuthResponse {
                ...
                data: Some(LoginResponse {
                    ...
                    user: LoginUserInfo {
                        ...
                        full_name: user.full_name,
                        subscription_tier: user.subscription_tier,
                        onboarding_status: OnboardingStatus::Verified.as_str().to_string(),
                    },
                }),
        ...
        Err(e) => {
            ...
            AuthError::DatabaseError("Database operation failed".to_string()).into_response()
        },
    }
}
...
pub async fn resend_verification(
        ...
        .await
    {
        ...
        Err(diesel::NotFound) => {
            ...
            let response = AuthResponse::<()> {
                success: true,
                data: None,
                message: "If the email exists, a new verification code has been sent".to_string(),
            };
            return (StatusCode::OK, Json(response)).into_response();
        },
        Err(e) => {
            tracing::error!("Database error during resend verification: {}", e);
            return AuthError::DatabaseError("Failed to get database connection".to_string())
                .into_response();
        },
    };
    ...
    if user.email_verified {
        return AuthError::ValidationError("Email is already verified".to_string()).into_response();
    }
    ...
    let cooldown = verification_service
        .get_resend_cooldown(&email)
        .await
        .unwrap_or(0);
    if cooldown > 0 {
        return AuthError::ValidationError(format!(
    ...
    {
        Ok(_) => {
            tracing::info!("Verification email resent to: {}", email);
...
pub async fn verification_status(
    ...
    let db_user: User = match dsl::users
        .filter(dsl::id.eq(user_uuid))
        .first(&mut conn)
        .await
    {
        Ok(u) => u,
        Err(e) => {
            tracing::error!("Failed to get user for verification status: {}", e);
    ...
    // Check resend status
    let resend_allowed = verification_service
        .check_resend_allowed(&db_user.email)
        .await
        .unwrap_or(false);

    let resend_cooldown = verification_service
        .get_resend_cooldown(&db_user.email)
        .await
        .unwrap_or(0);

    let response = VerificationStatusResponse {
        email_verified: db_user.email_verified,
        resend_allowed: resend_allowed && !db_user.email_verified,
        resend_cooldown_seconds: resend_cooldown,
        message: if db_user.email_verified {
            "Email is already verified".to_string()
        } else {
            "Email verification pending".to_string()
        },
    };
